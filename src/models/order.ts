...
export class OrderStore {
  ...
  async addProduct(product_id: number, order_id: number, quantity: number): Promise<Object> {
    try {
      const conn = await Pool.connect();
      ...
      if (status.rows[0].status == 'complete') {
        throw new Error("Couldn't add product because order status is complete")
      }
